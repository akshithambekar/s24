...
const CONFIG = {
  ...
  jupiterApiKeySecretId: process.env.JUPITER_API_KEY_SECRET_ID || "solana-autopilot-infra/jupiter-api-key",
...
async function loadJupiterApiKey(secretId, region) {
  ...
  const parsed = JSON.parse(resp.SecretString);
  if (!parsed.api_key) {
    throw new Error(`Secret ${secretId} missing api_key field`);
  }
...
function computeTickFields(solToUsdcQuote, usdcToSolQuote) {
  ...
  // Ensure bid <= ask (quotes can invert due to timing)
  const finalBid = Math.min(bidPrice, askPrice);
  const finalAsk = Math.max(bidPrice, askPrice);
  const midPrice = (finalBid + finalAsk) / 2;
  const spreadBpsRaw = ((finalAsk - finalBid) / midPrice) * 10000;
  const spreadBps = Math.max(0, Math.round(spreadBpsRaw));

  return {
    bidPrice: finalBid.toFixed(8),
    askPrice: finalAsk.toFixed(8),
...
async function run() {
  ...
  jupiterApiKey = await loadJupiterApiKey(CONFIG.jupiterApiKeySecretId, CONFIG.awsRegion);
  console.log("Jupiter API key loaded from Secrets Manager.");
  console.log("Starting Jupiter ingestion...");
